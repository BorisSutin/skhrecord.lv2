...
#define MAXFILESIZE INT_MAX - MAXRECSIZE // 2147352576  //2147483648-MAXRECSIZE
    ...
    float SCapture::dbToFloat(float db)
    {
        if (db <= -90.0f)
            return 0.0f;
    ...
    void always_inline SCapture::compute(int count, float *input0, float *output0)
    {
        ...
        *fcheckbox1 = int(fRecb2[0]);
        for (int i = 0; i < count; i++)
        {
            ...
            m_threshold = dbToFloat(*fthreshold);

            if (iSlow0 && !isRec)
            {
                if (fabs(fTemp0) >= m_threshold)
                {
                    isRec = true;
                    *state = 2.0f;
                }
        ...
        if (fabs(fRecb2[0]) >= m_threshold || isRec)
        {
    ...
    void always_inline SCapture::compute_st(int count, float *input0, float *input1, float *output0, float *output1)
    {
        ...
        *fcheckbox1 = int(fmax(fRecb2[0], fRecb2r[0]));
        for (int i = 0; i < count; i++)
        {
            ...
            if (iSlow0 && !isRec)
            {
                if (fabs(fTemp0) >= m_threshold || fabs(fTemp1) >= m_threshold)
                {
                    isRec = true;
                    *state = 2.0f;
                }
        ...
        if (fabs(fRecb2[0]) >= m_threshold || fabs(fRecb2r[0]) >= m_threshold || isRec)
        {
            *fbargraph = 20. * log10(fmax(0.0000003, fRecb2[0]));
    ...
    void SCapture::connect(uint32_t port, void *data)
    {
        ...
        case CLIP:
            ...
            break;
        case STATE:
            state = (float *)data; // , 0.0f, 0.0f, 1.0f, 1.0f
            break;
        case LMETER:
